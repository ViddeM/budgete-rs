...
#[cfg(feature = "server")]
use {crate::auth::session::current_household_id, crate::db::pool, crate::db_rows::GroupRow};

/// List all groups for the current household, newest first.
#[server]
...
/// Delete a group owned by the current household and remove all its transaction
/// assignments.
#[server]
pub async fn delete_group(group_id: Uuid) -> Result<(), ServerFnError> {
    ...
    sqlx::query("DELETE FROM transaction_groups WHERE group_id = $1")
        ...
        .await
        .map_err(|e| ServerFnError::new(e.to_string()))?;
    sqlx::query("DELETE FROM groups WHERE id = $1 AND household_id = $2")
        .bind(group_id)
        .bind(household_id)
        .execute(db)
        .await
...
/// group must belong to the current household.
#[server]
pub async fn add_to_group(tx_id: Uuid, group_id: Uuid) -> Result<(), ServerFnError> {
    ...
    sqlx::query(
        ...
        SELECT $1, $2
        WHERE EXISTS (SELECT 1 FROM transactions WHERE id = $1 AND household_id = $3)
          AND EXISTS (SELECT 1 FROM groups       WHERE id = $2 AND household_id = $3)
    ...
    sqlx::query(
        "DELETE FROM transaction_groups \
         WHERE transaction_id = $1 AND group_id = $2 \
           AND EXISTS (SELECT 1 FROM transactions WHERE id = $1 AND household_id = $3)",
    )
    .bind(tx_id)
    .bind(group_id)
    .bind(household_id)
